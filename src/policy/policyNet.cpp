...
int moveToLayer(int from, int to) {
    ...
    if (fromFile == toFile)
        return fromRank > toRank ? (fromRank - toRank + V_POS_OFFSET)
                                 : (toRank - fromRank + V_NEG_OFFSET);

    if (fromRank == toRank)
        return fromFile > toFile ? (fromFile - toFile + H_POS_OFFSET)
                                 : (toFile - fromFile + H_NEG_OFFSET);

    if (fromRank + fromFile == toRank + toFile)
        return fromFile > toFile ? (fromFile - toFile + D1_POS_OFFSET)
                                 : (toFile - fromFile + D1_NEG_OFFSET);

    if (fromRank - fromFile == toRank - toFile)
        return fromFile > toFile ? (fromFile - toFile + D2_POS_OFFSET)
                                 : (toFile - fromFile + D2_NEG_OFFSET);
