...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    highTTValue  = ttValue != VALUE_NONE && ttValue >= (probCutBeta + (1000 - std::min((int(std::log(tte->depth())) * 610), 900)));
    highDepth    = depth >= 12;

    probReduction = 3 + highTTValue + highDepth;
